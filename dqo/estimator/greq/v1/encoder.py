...
def encode_column(column: RelationColumn, db: Database):
...
def encode_selection(selection: SelectionNode, db: Database):
    ...
    if isinstance(selection, OrNode):
        ...
        left_encoded = encode_column(left, db) if isinstance(left, RelationColumn) else torch.zeros(COLUMN_ENCODING_LENGTH)
        right_encoded = encode_column(right, db) if isinstance(right, RelationColumn) else torch.zeros(COLUMN_ENCODING_LENGTH)
        # equijoins only current
        op_encoded = torch.tensor(operators_one_hot[selection.operator.upper()], dtype=torch.float64)

        # 4+9+3+9+1 = 26
        return torch.cat((node_encoding, left_encoded, op_encoded, right_encoded, torch.tensor([int(is_join)], dtype=torch.float64)))
...
def encode_query(db: Database, query: str, augment: bool = True) -> List[torch.Tensor]:
    ...
    #     encoded_perm.append(encode_rel_tree(p, db))
    #
    # return encoded_perm
...
if __name__ == '__main__':
    ...
    for index, row in tqdm(df.iterrows(), total=df.shape[0]):
        encode_query(db, row['query'], augment=True)
