...
def encode_col_hist_freq(
    ...
    freq = list(np.log([v if v else freq_pad for v in freq]))
    if padding:
        hist += ([np.log(hist_pad)] * padding)
        freq += ([np.log(freq_pad)] * (fixed_length - len(freq)))
...
def encode_projection(projection: ProjectionNode, db: Database) -> EncodedProjection:
    ...
    return EncodedProjection(tensor)
...
def encode_relation(relation: RelationNode, db: Database) -> EncodedRelation:
    ...
    Which gives us a vector of size: |S * 8 channels| + 5 + 1 + 1 , in the above case, where |S| is 7, we get a vector of size (49).
    :param relation:
    :param db:
    :return:
    """
    ...
    cum_column_stats = []
    for i in range(column_stats.shape[1]):
        stats = summarize_set(column_stats[:, i])
        cum_column_stats.append(np.nan_to_num(stats))
    tensor = torch.Tensor([
        *np.array(cum_column_stats).reshape(-1),
        np.log(column_types['string']) if 'string' in column_types else 0,
        np.log(column_types['number']) if 'number' in column_types else 0,
        ...
        np.log(column_types['bool']) if 'bool' in column_types else 0,
        np.log(column_types['time']) if 'time' in column_types else 0,
        np.log(table.stats.rows),
        np.log(table.stats.pages * table.stats.page_size)
    ])
    return EncodedRelation(tensor)
...
def encode_join(join: JoinNode, db: Database) -> EncodedJoin:
    ...
    for side in [left, right]:
        ...
        join_vector = [*join_vector, *col, *hist, *freq, int(db_col.stats.index)]

    tensor = torch.Tensor(join_vector + types_one_hot[db_col.data_type])
    return EncodedJoin(tensor)
...
def encode_selection(selection: SelectionNode, db: Database) -> EncodedSelection:
    ...
    return EncodedSelection(tensor)
...
def encode_node(node: RelationalNode, db: Database) -> EncodedNode:
    encoded = None
...
def encode_rel_tree(rel_tree: RelationalTree, db: Database) -> Tuple[Node[int], List[torch.Tensor]]:
    encoded_nodes = []

    rel_tree.optimize()
    indices = {}
    ns = {}
    for idx, node in enumerate(rel_tree.dfs()):
        node_id = id(node)
        ...
        if node.parent is not None:
            parent_idx = indices[id(node.parent)]
            n.set_parent(ns[parent_idx])

        ns[idx] = n

        encoded_nodes.append(encode_node(node, db).value)

    return ns[0], encoded_nodes


def encode_query(db: Database, query: str) -> Tuple[Node[int], List[torch.Tensor]]:
