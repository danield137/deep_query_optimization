...
class QueryEstimater:
    def __init__(
            ...
            dataset: Union[ExtendedQueriesDataset, str] = None,
            version=None
    ):
        """
        ...
        if dataset is not None:
            self.ds = dataset if isinstance(dataset, ExtendedQueriesDataset) else ExtendedQueriesDataset(dataset)
    ...
    def predict_query(self, q, plan):
        # handle different encoders
        if len(signature(self.encoder.encode_query).parameters) == 3:
            x = self.encoder.encode_query(self.schema, q, plan)
        else:
    ...
    def evaluate(
        ...
        if df is None:
            ...
            if type(dataset) is str:
                dataset = ExtendedQueriesDataset(dataset)
        ...
        with torch.no_grad():
            ...
            for index, row in tqdm(df.iterrows(), total=len(df), position=0, leave=True):
                q, true, plan = row['query'], row['runtime'], row['plan']
                pred = self.predict_query(q, plan)
...
def zero_rule_test_accuracy(test_ds, bucketed=True):
    df = ExtendedQueriesDataset(f'{test_ds}:optimized:test').load(include_bucket=True)
    if bucketed:
